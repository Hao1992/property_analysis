# Beyond Price — Spanish Property Intelligence
...
This started as an attempt to build a viable product around Spanish property due-diligence. It didn't find a path to revenue, so it's no longer actively developed as a commercial project — but the engineering behind it (city-aware tax/legal logic, an 8-dimension scoring engine, a documented incident-driven review protocol) is solid and may be useful to others. Open-sourced under MIT so anyone can learn from it, fork it, or build on it. Issues and PRs are welcome, but expect best-effort, not active maintenance.
...
- [x] Madrid and Valencia support (city-aware tax/legal logic; safety data stays BCN-only)
...
- [ ] Safety/crime and neighbourhood-trajectory data sources for Madrid and Valencia

---

## Contributing

Bug reports and PRs welcome. See [CONTRIBUTING.md](CONTRIBUTING.md) before opening a PR — this project had a real quality incident early on (10 "done" features, 18 bugs, 3 production crashes) and the repo's `CLAUDE.md` documents the review protocol adopted afterward. It's a good read even if you're not using Claude Code.
